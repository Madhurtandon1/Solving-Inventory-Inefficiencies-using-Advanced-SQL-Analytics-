...
--  CTE 3: Approximate inventory age (days between first and last inventory record)
InventoryAge AS (
    SELECT 
        product_id,
        store_id,
        region,
        DATEDIFF(DAY, MIN(date), MAX(date)) AS inventory_span_days,
        AVG(CAST(inventory_level AS FLOAT)) AS avg_inventory_age
    FROM Inventory
    GROUP BY product_id, store_id, region
)
